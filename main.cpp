...
void maksMinAlfaBeta(BoardState& state, int depth, int alpha, int beta, bool maximizingPlayer);
...
bool checkWin(const BoardState& state) {
    ...
    for (int i = 0; i < 3; ++i) {
        if (state.board[i * 3] != 0 && state.board[i * 3] == state.board[i * 3 + 1] && state.board[i * 3 + 1] == state.board[i * 3 + 2]) {
            return true;
        }
